...
- `read_chunk_window` — doc_id + anchor_chunk_id (+ window_before/window_after>=0); проверяет лимит окна, ходит в Retrieval Service `/internal/retrieval/chunks/window` и режет текст до `max_text_bytes`.
...
- Все тексты обрезаются по `max_text_bytes`; окно чанков ограничено `max_chunk_window`.

## 5. Configuration (`MCP_PROXY_*`)
`app_name`, `host/port/log_level`, `max_pages_per_call`, `max_text_bytes`, `rate_limit_calls`, `rate_limit_tokens`, `mock_mode`, `retrieval_window_url`, `retrieval_timeout`, `max_chunk_window`, `blocklist_keywords`.
